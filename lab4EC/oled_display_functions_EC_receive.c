 ...
 * lab4 extra credit-receive ball coordinates from remote processor via Xbee and display
 * locally on oled display
 */
...
void UART1IntHandler(void) {
    ...
    int fromUARTChar1, fromUartChar2;
        while(UARTCharsAvail(UART1_BASE))
        {
            ...
            //update ballX and Y and redraw ball
...
void SysTick_Handler (void) {
    ...
    if (accelX>-30 && accelX<32) {
        if (accelX<0) {
            if (ballX<120) {//switch to 120, switch next to 8
                ballX=ballX-accelX/5;
            }
            
        }
        else{
            if(ballX>8){//120 originally
                ballX-=accelX/5;
            }
        }
    }
    if (accelY>-28 && accelY<32) {
        if (accelY<0) {
            if (ballY>8) {
                ballY=ballY+accelY/5;
            }
...
void initI2C(void){
    ...
    //write to reg 0x07 for active mode
    I2CMasterSlaveAddrSet(I2C0_BASE,SLAVE_ADDRESS,false);//false to send
    I2CMasterDataPut(I2C0_BASE,0x07);
    I2CMasterControl(I2C0_BASE,I2C_MASTER_CMD_BURST_SEND_START);
    while (I2CMasterBusy(I2C0_BASE)) {;}//wait until done sending
    ...
    //configure interrupt type for PB5 (falling edge) after i2c setup
